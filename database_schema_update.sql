...
-- First, add unique constraint on agent_name if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                   WHERE table_name = 'agent_status' 
                   AND constraint_type = 'UNIQUE' 
                   AND constraint_name LIKE '%agent_name%') THEN
        ALTER TABLE agent_status ADD CONSTRAINT agent_status_agent_name_unique UNIQUE (agent_name);
    END IF;
END $$;

-- Insert sample agent status records
INSERT INTO agent_status (agent_name, status, last_activity) VALUES
    ('alert_receiver_ai', 'active', NOW()),
    ('false_positive_checker_ai', 'active', NOW()),
    ('severity_analyzer_ai', 'active', NOW()),
    ('context_gatherer_ai', 'active', NOW()),
    ('response_coordinator_ai', 'active', NOW())
